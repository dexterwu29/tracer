...
class SendSmsForm(forms.Form):
    ...
    def clean_mobile_phone(self):
        ...
        if not template_id:
            ...
            # 虽然写法等价于上面，但是这样出错了直接抛出错误就不往下走了 ❌
            raise ValidationError("短信模板错误")

        # 登录时数据库应该有“该手机号”，注册是应该无
        exists = models.UserInfo.objects.filter(mobile_phone=mobile_phone).exists()
        if tpl == 'login':
            if not exists:
        ...
        if sms["result"] != 0:
            return ValidationError("短信发送失败，{}".format(sms["errmsg"]))
...
class LoginForm(BootStrapForm, forms.Form):
    ...
    password = forms.CharField(label='密码', widget=forms.PasswordInput(render_value=True))  # render_value=True 密码填错也不会清空
